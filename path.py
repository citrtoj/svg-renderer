...
class PathCommands(Enum):
    ...
    def get_length(command: str):
        ...
        if command_upper not in PathCommands.__members__:
            raise ValueError(f"Invalid command {command}; not in list {PathCommands.__members__.keys()}")
        return PathCommands.__members__[command_upper].value


def parse_path_commands(path_data: str):
    commands = []
    ...
    def add_commands(command, numbers):
        if len(numbers) % 2 != 0:
            raise ValueError("numbers should be of length multiple of 2")
        for i in range(len(numbers) // 2):
            commands.append((command, (numbers[i], numbers[i + 1])))

    for match in matches:
        if match[0]:
            if current_command is not None:
                args_len = PathCommands.get_length(current_command)

                if args_len == 0:
                    if len(current_numbers) == 0:
                        commands.append((current_command, tuple()))
                    else:
                        raise ValueError(
                            f"Invalid command {current_command} with too many numbers ({len(current_numbers)})"
                        )

                else:
                    if len(current_numbers) % args_len != 0:
                        print(args_len)
                        print(len(current_numbers))
                        raise ValueError(
                            f"Not enough numbers ({len(current_numbers)}) for all commands of same type {current_command}"
                        )
                    for i in range(len(current_numbers) // args_len):
                        commands.append(
                            (
                                current_command,
                                tuple(current_numbers[j] for j in range(i * args_len, (i + 1) * args_len)),
                            )
                        )
...
if __name__ == "__main__":
    ...
    print(parse_path_commands(path_data))
